    ...
    <div className="grid h-full min-h-0 gap-4 xl:grid-cols-[300px_minmax(0,1fr)_320px]">
      <aside className="min-h-0 rounded-[1.8rem] border border-white/8 bg-white/[0.025] p-4">
        ...
        <div className="space-y-2">
          {threads.map((thread) => (
            <button key={thread.id} type="button" onClick={() => { setSelectedThreadId(thread.id); setTargetAgentName(thread.agent?.name || agents[0]?.name || ''); setSelectedContext([]); onThreadChange?.(thread.id); }} className={`w-full rounded-[1.4rem] border px-4 py-3 text-left transition ${selectedThreadId === thread.id ? 'border-white/20 bg-white/[0.06]' : 'border-white/8 bg-black/30 hover:border-white/15'}`} data-testid={`chat-thread-item-${thread.id}`}>
      ...
      <section className="grid min-h-0 grid-rows-[auto_1fr_auto] overflow-hidden rounded-[1.8rem] border border-white/8 bg-white/[0.025]">
        ...
        <div className="border-t border-white/6 px-5 py-4">
          ...
          <div className="mt-3 rounded-[1.3rem] border border-white/7 bg-black/35 p-3" data-testid="chat-context-panel">
      ...
      <aside className="rounded-[1.8rem] border border-white/8 bg-white/[0.025] p-4">
        ...
        <div className="mt-4 rounded-[1.4rem] border border-white/7 bg-black/35 p-4" data-testid="chat-selected-message-inspector">
          <p className="text-sm font-medium text-white">Mensaje seleccionado</p>
          {selectedMessage ? (
            <>
        ...
        <div className="mt-4 rounded-[1.4rem] border border-white/7 bg-black/35 p-4" data-testid="chat-linked-resources-panel">
          ...
          <div className="mt-3 space-y-3">
            ...
            {Array.isArray(selectedMessageMetadata?.executedActions) && selectedMessageMetadata?.executedActions.length ? (selectedMessageMetadata.executedActions as Array<Record<string, unknown>>).map((item, index) => (
              <article key={`${index}-${String(item.label || item.kind || 'action')}`} className="rounded-[1.1rem] border border-white/7 bg-white/[0.03] p-3" data-testid={`linked-action-${index}`}>
                <p className="text-sm text-white">{String(item.label || item.kind || 'action')}</p>
